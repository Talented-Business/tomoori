...
class wf_dhl_woocommerce_shipping_admin_helper {
                ...
                try {
                    ...
                    if (elex_dhl_get_product_length($values['data']) && elex_dhl_get_product_height($values['data']) && elex_dhl_get_product_width($values['data'])) {
                        ...
                        $group['Dimensions'] = array(
                            'Length' => max(1, round(wc_get_dimension($dimensions[2], $this->dimension_unit))),
                            'Width' => max(1, round(wc_get_dimension($dimensions[1], $this->dimension_unit))),
                            'Height' => max(1, round(wc_get_dimension($dimensions[0], $this->dimension_unit))),
    ...
    * @return mixed array
    */
    private function set_product_properties_dhl_commercial_invoice($product_id, $order_item_data){
        ...
        $product_details['weight'] = $product->get_weight();
    ...
    */
    ...
    public function generate_commercial_invoice($order_id, $packages, $shipper, $toaddress, $document_type = 'commercial', $archive_ref = array(), $pickup_confirmation_number = '') {
        ...
        if (isset($this->settings['include_woocommerce_tax']) && ($this->settings['include_woocommerce_tax'] == 'yes')) {
            $total_shipping_tax_on_order = wf_get_total_tax_on_order($order);
        }
        ...
        $total_shipping_tax_on_order -= $tax_on_products;
        ...
        $package_details = array(
            ...
            'shipment_content' => isset($this->settings['label_contents_text'])? $this->settings['label_contents_text']: '',
    ...
    private function generate_return_commercial_invoice($packages, $shipper, $toaddress, $selected_items = "", $archive_ref) {
        ...
        $package_details = array(
            ...
            'shipment_content' => (isset($this->settings['label_contents_text']) && !empty($this->settings['label_contents_text']))? $this->settings['label_contents_text']: '',
            'shipment_comments' => !empty($this->label_comments_text)? $this->label_comments_text: ''
        );
    ...
    public function get_package_signature($products, $orderid) {
        ...
        $order_delivery_signature = get_post_meta($orderid, 'wf_dhl_delivery_signature', true);
        if($order_delivery_signature == 'yes'){
            $higher_signature_option = 1;            
        }      
    ...
    */
    ...
    public function get_order_items_total($package_contents){
        ...
        foreach ($package_contents as $item_key => $item_value) {
            ...
            $item_price = $item_data->get_sale_price();
            if (empty($item_sale_price)) {
                $item_price = $item_data->get_regular_price();
            }
    ...
    private function get_dhl_requests($dhl_packages, $package) {
        ...
        $order_items_total = $this->get_order_items_total($package['contents']);
        ...
        $order_subtotal =  round($total_after_discount);
        ...
        $order_dutiable_amount = $order_subtotal != 0? $order_subtotal: $order_items_total;
        $order_dutiable_amount = round((int)$order_dutiable_amount);
        $shipping_service_rates = get_post_meta($orderid,'_wf_dhl_available_services',true);
        $shipping_monetory_value = $shipping_service_rates[$this->service_code]['cost']; 
        $order_dutiable_amount += $shipping_monetory_value;

        $dutiable_content = $is_dutiable == "Y" ? "<Dutiable><DeclaredValue>{$order_dutiable_amount}</DeclaredValue><DeclaredCurrency>{$order_currency}</DeclaredCurrency>" : "";
        ...
        if($bulk_create_shipment){
            ...
            $stored_ordered_ids_with_no_default_shipment_service = get_option('orders_with_no_default_shipment_service');
            ...
            update_option("orders_with_no_default_shipment_service", $stored_ordered_ids_with_no_default_shipment_service);
        }
        ...
        $shipping_method_data = $this->order->get_shipping_methods();
        ...
        if (!empty($shipping_method_data)) {
            ...
            if (!empty($shipping_method_data)) {
                ...
                if (!empty($shipping_method_meta_data)) {
        ...
        if ($is_dutiable == 'Y') {
            ...
            foreach ($package['contents'] as $i => $item) {
                ...
                $export_line_item .= '  <Value>' . $item_value . '</Value>';
                ...
                $export_line_item .= '  <Weight><Weight>' . round($xa_send_dhl_weight, 3) . '</Weight><WeightUnit>' . $this->product_weight_unit . '</WeightUnit></Weight>';
...
XML;
    ...
    */
    ...
    function wf_pickup_request_handler($order) {
        $output = $this->wf_pickup_request($order);
        ...
        if (isset($xml->Note->ActionNote) && $xml->Note->ActionNote == 'Success') {
            ...
            update_post_meta($order_id, '_wf_dhl_pickup_shipment', array("pickup_confirmation_number" => $ConfirmationNumber, "next_pickup_date" => $NextPickupDate, "ready_by_time" => $ReadyByTime));
    ...
    private function wf_pickup_request($order) {
        $order_id = elex_dhl_get_order_id($order);
        ...
        if (!empty($airwaybill_number) && !empty($dhl_packages)) {
            ...
            if($pickup_day == 'Sat'){
                $pickup_date = date('Y-m-d', strtotime("+2 days", current_time('timestamp')));
            }else if($pickup_day == 'Sun'){
                $pickup_date = date('Y-m-d', strtotime("+1 day", current_time('timestamp')));
            }
...
XML;
    ...
    private function get_dhl_shipping_return_label_requests($dhl_packages, $package) {
        ...
        if($bulk_create_return_shipment){
            ...
            update_option("orders_with_no_default_shipment_service", $orders_with_no_default_shipment_service);
        }
        ...
        $toaddress = array(
            ...
            'company' => str_replace("&", '&amp;', elex_dhl_get_order_shipping_company($this->order)),
        ...
        if ($this->plt && $is_dutiable == 'Y') {
            update_option('PLT_enabled_express_dhl_elex', true);
            $sample_base64_encoded_pdf = $this->generate_return_commercial_invoice($dhl_packages, $toaddress, $shipper, $selected_items, $archive_ref);
            $special_service .= "<SpecialService><SpecialServiceType>WY</SpecialServiceType></SpecialService>";
        ...
        $this->settings['return_shipment_address']['return_receiver_country_name'] = isset(WC()->countries->countries[$this->settings['return_shipment_address']['return_receiver_country']]) ? WC()->countries->countries[$this->settings['return_shipment_address']['return_receiver_country']] : $package['destination']['country'];
        ...
        if ($is_dutiable == 'Y') {
            ...
            foreach ($package['contents'] as $i => $item) {
                ...
                $export_line_item .= '  <Weight><Weight>' . round($item['data']->get_weight(), 3) . '</Weight><WeightUnit>' . $this->product_weight_unit . '</WeightUnit></Weight>';
        ...
        $return_shipment_address = $this->settings['return_address_different'] !== 'yes'? array(
            'return_receiver_person_name' => $shipper['contact_person_name'],
            'return_receiver_company_name' => $shipper['company_name'],
            'return_receiver_phone_number' => $shipper['contact_phone_number'],
            'return_receiver_email' => $shipper['contact_email'],
            'return_receiver_street' => $shipper['address_line'],
            'return_receiver_street_2' => $shipper['address_line2'],
            'return_receiver_city' => $shipper['city'],
            'return_receiver_state' => $shipper['division'],
            'return_receiver_postcode' => $shipper['postal_code'],
            'return_receiver_country' => $shipper['country_code'],
            'return_receiver_country_name' => $shipper['country_name']
...
<req:ShipmentRequest xmlns:req="http://www.dhl.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.dhl.com ship-val-global-req-6.0.xsd" schemaVersion="6.0">
    ...
    <Consignee>
        <CompanyName>{$return_shipment_address['return_receiver_company_name']}</CompanyName>
        <AddressLine>{$return_shipment_address['return_receiver_street']}</AddressLine>
        <AddressLine>{$return_shipment_address['return_receiver_street_2']}</AddressLine>
        <City>{$return_shipment_address['return_receiver_city']}</City>
        <Division>{$return_shipment_address['return_receiver_state']}</Division>
        <PostalCode>{$return_shipment_address['return_receiver_postcode']}</PostalCode>
        <CountryCode>{$return_shipment_address['return_receiver_country']}</CountryCode>
        <CountryName>{$return_shipment_address['return_receiver_country_name']}</CountryName>
        <Contact>
            <PersonName>{$return_shipment_address['return_receiver_person_name']}</PersonName>
            <PhoneNumber>{$return_shipment_address['return_receiver_phone_number']}</PhoneNumber>
            <Email>{$return_shipment_address['return_receiver_email']}</Email>
        </Contact>
    </Consignee>
    {$dutiable_content}
    {$shipment_details}
            ...
            foreach ($dhl_packages as $group_key => $package_group) {
                ...
                foreach ($package_group as $key => $parcel) {
                    ...
                    for($parcel_quantity = 1; $parcel_quantity <= $parcel['quantity']; $parcel_quantity++){
                        ...
                        $piece_weight = round($parcel['Weight']['Value'], 2) == 0? '0.01':  round($parcel['Weight']['Value'], 2);
...
XML;
    ...
    private function get_result($request) {
        ...
        if($shipment_type_return){
            ...
            $this->generate_return_commercial_invoice_with_awb = $this->generate_return_commercial_invoice($this->packages_dhl, $return_shipment_shipper, $return_shipment_reciever, $selected_items, $archive_ref);
        ...
        if (!$xml) {
            ...
            if($shipment_type_return){
                $response = array(
                    ...
                    'CommercialInvoice' => (string) $this->generate_return_commercial_invoice_with_awb,
                ...
                $response = array(
                    'ShipmentID' => (string) $xml->AirwayBillNumber,
                    'LabelImage' => (string) $xml->LabelImage->OutputImage,
                    'CommercialInvoice' => (string) $this->generate_commercial_invoice_with_awb,
                    'ErrorMessage' => $this->errorMsg,
                );
            }
