...
class wf_dhl_woocommerce_shipping_method extends WC_Shipping_Method {
    ...
    public function is_available( $package ) {
        ...
        if($post_data_string != ''){
            ...
            $dhl_delivery_signature_checkout = isset($post_data_string['wf_dhl_delivery_signature'])? 'yes': 'no';
            update_option('wf_dhl_delivery_signature_enabled_checkout_no_real_time_enabled', $dhl_delivery_signature_checkout);
        }else{
            /* while creating order */
            $dhl_insurance_checkout = isset($_POST['wf_dhl_insurance'])? 'yes': 'no';
            update_option('wf_dhl_insurance_enabled_checkout_no_real_time_enabled', $dhl_insurance_checkout);
            $dhl_delivery_signature_checkout = isset($_POST['wf_dhl_delivery_signature'])? 'yes': 'no';
            update_option('wf_dhl_delivery_signature_enabled_checkout_no_real_time_enabled', $dhl_delivery_signature_checkout);
        }
    ...
    public function add_custom_actions_my_account($actions, $order){
        if ( (isset($this->settings['option_print_label_by_customers']) && $this->settings['option_print_label_by_customers'] == 'yes') && $order->has_status( 'completed' ) ) {
    ...
    public function generate_wf_dhl_tab_box_html() {
        ...
        $tab = (!empty($_GET['subtab'])) ? esc_attr($_GET['subtab']) : 'general';
                ...
                switch ($tab) {
                    ...
                    case "licence":
                        ...
                        include 'html-wf-dhl-express-addon.php';
                        echo '</div>';
                        break;
                }
                echo '
       ...
       public function wf_dhl_shipping_page_tabs($current = 'general')
       {
            ...
            $auto_label_add_on =  "<small style='color:green;font-size:xx-small;'>(Settings)</small>";
            $tabs = array(
            ...
            if(ELEX_DHL_EXPRESS_AUTO_LABEL_GENERATE_ADDON_WOOCOMMERCE_EXTENSION){
                $tabs['auto-generate-add-on'] =  __("Auto Label Generate Add-on ".$auto_label_add_on,'wf-shipping-dhl');
            }
    ...
    **/
    ...
    private function per_item_shipping( $package ) {
        ...
        foreach ($package['contents'] as $item_id => $values) {
            ...
            if (!$values['data']->get_weight()) {
                $this->debug(sprintf(__('Product %s is missing weight. Aborting.', 'wf-shipping-dhl'), $values['data']->get_name()), 'error');
                return;
            }
            ...
            if(wc_get_weight($values['data']->get_weight(), $this->weight_unit)<0.001){
                ...
                $xa_item_value = wc_get_weight($values['data']->get_weight(), $this->weight_unit);
                $xa_per_item_weight = $xa_item_value;
            }
    ...
    */
    ...
    private function get_dhl_requests($dhl_packages, $package) {
        ...
        */
        ...
        $total_insurance_value = apply_filters('wc_aelia_cs_convert', $total_insurance_value, $this->shop_currency, get_woocommerce_currency()) != ''? apply_filters('wc_aelia_cs_convert', $total_insurance_value, $this->shop_currency, get_woocommerce_currency()) : '';
        ...
        $delivery_signature_details = '';
        
        if(isset($package['dhl_delivery_signature']) && ($package['dhl_delivery_signature'] == 'yes')){
            update_option("wf_dhl_delivery_signature", 'yes');
        }else{
            update_option("wf_dhl_delivery_signature", 'no');
        }

        $special_service_type_details = '';
        if(($additional_insurance_details != '') || ($delivery_signature_details != '')){
            $special_service_type_details = "<QtdShp><QtdShpExChrg>";
            if($delivery_signature_details != ''){
                $special_service_type_details .= $delivery_signature_details;
            }
        ...
        if(in_array($package['destination']['postcode'], $canary_islands_postcodes)){
...
XML;
    ...
    */
    ...
    public function calculate_shipping( $packages = array() ) {
        ...
        $calculate_shipping_rates = apply_filters('calculate_shipping_in_shop_page_express_dhl_elex', false);// Hook to enable/disable calculating shipping rates on the shop while adding products to the cart
        if( !$calculate_shipping_rates && !( is_cart() || is_checkout() ) ){
            return;
        }
        ...
        foreach($parcels as $parcel){
            ...
            /* Handling insurance and Signature charges */
            if($this->settings['insure_contents_chk'] == 'yes'){
                /* on checkout page */
                if($str != '' && strpos($http_referer, 'checkout') > 0){
                    $parcel['dhl_insurance'] = isset($str['wf_dhl_insurance'])? 'yes': 'no';
                    $parcel['dhl_delivery_signature'] = isset($str['wf_dhl_delivery_signature'])? 'yes': 'no';
                }else if(is_cart() && $this->settings['insure_contents'] == 'yes'){
                    $parcel['dhl_insurance'] = 'yes';
                }else{
                    /* while creating order */
                    $parcel['dhl_insurance'] = isset($_POST['wf_dhl_insurance'])? 'yes': 'no';
                    $parcel['dhl_delivery_signature'] = isset($_POST['wf_dhl_delivery_signature'])? 'yes': 'no';
                }
            }else if($this->settings['insure_contents'] == 'yes'){
                $parcel['dhl_insurance'] = 'yes';
            }
    ...
    */
    ...
    private function process_result($result, $defined_req = array(), $dhl_inurance) {
        ...
        if ($response && !empty($response_services)) {
            foreach ($response_services as $response_service) {
                ...
                if (!in_array($rate_code,$processed_ratecode)) {
                    ...
                    if($rate_cost > 0) {
                        $this->prepare_rate($rate_code, $rate_id, $rate_name, $rate_cost, $delivery_date_time, $rate_cost_weight, $extra_charges['insurance_charge'], $extra_charges['remote_area_surcharge'], $shipping_rates_source_currency);
                    }
                }
            }
        }else if($result && !empty($result->GetQuoteResponse->Note)){
            foreach($result->GetQuoteResponse->Note->Condition as $condition){
                $this->debug(sprintf(__('DHL Error:  %s', 'wf-shipping-dhl'), htmlspecialchars($condition->ConditionData)), 'error');
                return;
            }
        }
    }
    ...
    private function get_dhl_extra_charges($response_service){
        ...
        if(isset($response_service['QtdShpExChrg'])){
            ...
            foreach ($extra_shipping_charges as $extra_shipping_charge) {
                if(isset($extra_shipping_charge['GlobalServiceName'])){
                    if($extra_shipping_charge['GlobalServiceName'] == 'REMOTE AREA DELIVERY'){
                        $extra_charges['remote_area_surcharge'] = $extra_shipping_charge['ChargeValue'];
                    }

                    if($extra_shipping_charge['GlobalServiceName'] == 'SHIPMENT INSURANCE'){
                        $extra_charges['insurance_charge'] = $extra_shipping_charge['ChargeValue'];
                    }
                }
            }
        }
    ...
    private function prepare_rate($rate_code, $rate_id, $rate_name, $rate_cost, $delivery_time, $rate_cost_weight = '0', $dhl_insurance, $dhl_remote_area_surcharge, $shipping_rates_source_currency) {
        ...
        $extra_charge_basic = $rate_cost - $rate_cost_weight;
        ...
        if($this->aelia_activated){
            $rate_cost_weight = apply_filters('wc_aelia_cs_convert', $rate_cost_weight, $this->shop_currency, get_woocommerce_currency()) != ''? apply_filters('wc_aelia_cs_convert', $rate_cost_weight, $this->shop_currency, get_woocommerce_currency()) : '';
            $extra_charge_basic = apply_filters('wc_aelia_cs_convert', $extra_charge_basic, $this->shop_currency, get_woocommerce_currency()) != ''? apply_filters('wc_aelia_cs_convert', $extra_charge_basic, $this->shop_currency, get_woocommerce_currency()) : '';
        }

        $shipping_service_meta_data = array('dhl_delivery_time'=>$delivery_time,'weight_charge'=>floatval($rate_cost_weight),'extra_charge'=>$extra_charge_basic);
        ...
        if($this->general_settings['show_dhl_extra_charges'] == 'yes' && $this->general_settings['insure_contents'] == 'yes' && $this->general_settings['show_dhl_insurance_charges'] == 'yes'){
            ...
            $shipping_service_meta_data = array('dhl_delivery_time'=>$delivery_time,'weight_charge'=>floatval($rate_cost_weight),'insurance_charge'=>$dhl_insurance,'extra_charge'=>$extra_charge);
        }
        ...
        if($this->general_settings['show_dhl_extra_charges'] == 'yes' && $this->general_settings['show_dhl_remote_area_surcharge'] == 'yes'){
            if($this->general_settings['insure_contents'] == 'yes' && $this->general_settings['show_dhl_insurance_charges'] == 'yes'){
                ...
                $shipping_service_meta_data = array('dhl_delivery_time'=>$delivery_time,'weight_charge'=>floatval($rate_cost_weight),'insurance_charge'=>$dhl_insurance,'remote_area_surcharge'=>$dhl_remote_area_surcharge, 'extra_charge'=>$extra_charge); 
            }else{
                $extra_charge = $extra_charge_basic - $dhl_remote_area_surcharge;
                $shipping_service_meta_data = array('dhl_delivery_time'=>$delivery_time,'weight_charge'=>floatval($rate_cost_weight),'remote_area_surcharge'=>$dhl_remote_area_surcharge, 'extra_charge'=>$extra_charge); 
            }
        }
