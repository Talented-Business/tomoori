...
class WF_Tracking_Admin_DHLExpress
{
...
XML;
        ...
        if ( is_wp_error( $result ) ) {
               $tracking_array['status'] = 'HTTP Failure';
        }
        else
        {
            ...
            if(isset($xml->AWBInfo->Status->ActionStatus))
            {
                ...
                {   
                    ...
                    foreach ($xml->AWBInfo->ShipmentInfo->ShipmentEvent as $value) {
                        $tracking_array['shipment'][$i]['date'] =  (string)$value->Date;
                        $tracking_array['shipment'][$i]['time'] =  (string)$value->Time;
                        $tracking_array['shipment'][$i]['desc'] =  (string)$value->ServiceEvent->Description;
                        $tracking_array['shipment'][$i]['area'] =  (string)$value->ServiceArea->Description;
                        $tracking_array['shipment'][$i]['code'] =  (string)$value->ServiceEvent->EventCode;
                        $i++;
                    }
                }
            }
    ...
    function wf_add_tracking_info_to_email( $order, $sent_to_admin = false, $plain_text = false ) {
        $shipment_result_array  = get_post_meta( elex_dhl_get_order_id($order), self::SHIPMENT_RESULT_KEY, true );
    ...
    <?php
            ...
            foreach ($saved_tracking_data as $key => $value) {
                foreach ($value['shipment'] as $key => $shipment) {
    ...
    ?>
                ...
                <img class="help_tip" style="float:none;font-size: 17.5px;padding: 10px 6px 12px;" data-tip="<?php _e( 'Comma separated, in case of multiple shipment ids for this order.', 'wf-shipping-dhl' ); ?>" src="<?php echo WC()->plugin_url();?>/assets/images/help.png" height="16" width="16" /><br>
...
new WF_Tracking_Admin_DHLExpress();