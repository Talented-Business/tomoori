...
class wf_dhl_woocommerce_shipping_admin{
    ...
    public function __construct(){
        ...
        $last_order_wf_dhl_delivery_signature = false;
        $last_order_wf_dhl_insurance = false;

        if(isset($this->settings['enabled']) && ($this->settings['enabled'] === 'yes')){
            $last_order_wf_dhl_delivery_signature = get_option('wf_dhl_delivery_signature');
            $last_order_wf_dhl_insurance = get_option('wf_dhl_insurance');
        }else{
            $last_order_wf_dhl_delivery_signature = get_option('wf_dhl_delivery_signature_enabled_checkout_no_real_time_enabled');
        ...
        if(is_object($this->last_order_id)){
            ...
            $last_order_post_delivery_signature = get_post_meta($this->last_order_id->ID, 'wf_dhl_delivery_signature', true);
        }

        if(empty($last_order_post_meta_insurance) && empty($last_order_post_delivery_signature) && is_object($this->last_order_id)){
            update_post_meta($this->last_order_id->ID, 'wf_dhl_insurance', $last_order_wf_dhl_insurance);
            update_post_meta($this->last_order_id->ID, 'wf_dhl_delivery_signature', $last_order_wf_dhl_delivery_signature);
        }
            ...
            foreach ($response_services as $response_service) {
                ...
                if(!empty($extra_shipping_charges)){
                    foreach ($extra_shipping_charges as $extra_charge) {
                        ...
                        if(isset($extra_charge['GlobalServiceName']) && $extra_charge['GlobalServiceName'] == 'SHIPMENT INSURANCE'){
    ...
    private function wf_get_cost_based_on_currency($qtdsinadcur, $default_charge) {
        if (!empty($qtdsinadcur)) {
            foreach ($qtdsinadcur as $multiple_currencies) {
                if(isset($multiple_currencies['CurrencyCode'])){
        ...
        if ($action == 'create_shipment_dhl') {
            ...
            if(isset($_REQUEST['post']) && !empty($_REQUEST['post']))
            {
                ...
                if(get_option('default_shipment_service') == 'yes'){
                    ...
                    $orders_ids_with_no_default_shipment_service = get_option('orders_with_no_default_shipment_service');
    ...
    public function generate_proforma_invoice(){
        ...
        if(!empty($order_id)){
            ...
            if(!empty($proforma_invoice)){
                update_option("is_elex_dhl_express_proforma_invoice_generated", true);
    ...
    public function print_proforma_invoice(){
        ...
        if(!empty($order_id)){
            ...
            if(!empty($proforma_invoice)){
                ...
                update_option("is_elex_dhl_express_proforma_invoice_generated", false);
                wp_redirect( admin_url( '/post.php?post='.$order_id.'&action=edit') );
                exit;
            }
        }
        ...
        if (!empty($shipmentIds)) {
            ...
            foreach($shipmentIds as $shipmentId) {
                ...
                if(!empty($stored_pickup_shipment))
                {
                    ...
                    $pickup_date = isset($stored_pickup_shipment['next_pickup_date']) ? " <b>Next Possible Pickup Date:</b> " . (string) $stored_pickup_shipment['next_pickup_date'] : '';  
                ...
                if(empty($return_shipmentIds))
                {
                    ...
                    else
                    {
                        ...
                        echo '<li>choose Return service:<br><select class="wc-enhanced-select" style="width:40%;padding:5px" id="dhl_express_manual_return_service">';
                            if($this->show_front_end_shipping_method && $this->show_front_end_shipping_method === 'yes' && !empty($selected_service))
                            {
                                echo '<option value="'.$selected_service.'" selected="true" >'.$this->services[$selected_service].'</option>';
                            }else
                            {
                            ...
                            ?>
                            <li>
                                Shipment Content: <input type="text" placeholder="Enter Shipment Contents to ship" style="width:50%;" id="wf_dhl_shpment_content" name="wf_dhl_shpment_content" value="<?php echo $this->label_contents_text; ?>">  
                            </li>
                            <?php $label_comments_text = get_post_meta($post->ID, 'shipment_comments_express_dhl_elex', true);?>
                            <li>
                                Shipper Comments: <input type="text" placeholder="Enter Comments for the shipment" style="width:50%;" id="shipment_comments_express_dhl_elex" name="shipment_comments_express_dhl_elex" value="<?php echo $label_comments_text; ?>">  
                            </li>
                            <?php
                            echo '<hr style="border-color:#c9c9c9"></li>';
                        ?>
            ...
            if(empty($stored_packages)  &&  !is_array($stored_packages)){
                ...
                $return_service_data = get_post_meta($post->ID,'_wf_dhl_available_services',true);
                ...
                if(!empty($return_service_data))
                {
                    foreach ($return_service_data as $key => $value) {
                        ...
                        if($this->show_front_end_shipping_method && $this->show_front_end_shipping_method === 'yes' && !empty($selected_service) && array_key_exists($selected_service,$return_service_data))
                        {
                            if( $selected_service === $value['id'] || $selected_service === $value['label'] )
                            {
                                ...
                                $value_cost = apply_filters('wc_aelia_cs_convert', $value['cost'], $this->shop_currency, get_woocommerce_currency()) != ''? apply_filters('wc_aelia_cs_convert', $value['cost'], $this->shop_currency, get_woocommerce_currency()) : '';
                        ...
                        {
                            if(array_key_exists($selected_service,$return_service_data )){
                                if($selected_service === $value['id'] || $selected_service === $value['label']){
                                    echo '<td style="padding-left: 5px;padding-bottom: 3px;"><input name="wf_service_choosing_radio" id="wf_service_choosing_radio" value="'.$value['id'].'" type="radio" checked="true" ></td>';
                                }else{
                ...
                ?>
                <li>
                    <?php if($this->settings['option_generate_proforma_invoice'] == 'yes'):?>
                    <?php $is_proforma_invoice_generated = get_option("is_elex_dhl_express_proforma_invoice_generated", false);?>
