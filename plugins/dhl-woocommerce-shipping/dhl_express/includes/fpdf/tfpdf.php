 ...
 *******************************************************************************/
    function __construct($orientation = 'P', $unit = 'mm', $size = 'A4') {
        ...
        $this->PDFVersion = '1.3';
        $this->system_os = php_uname();
        $this->ttffontsfile = '';
        $this->fontfilepath = '';
        $this->_setfontsfilepath();
    }

    function changeToSystemPathFormat($path) {
        if (strpos($this->system_os, 'Windows')) {
    ...
    function AddFont($family, $style = '', $file = '', $uni = false) {
        ...
        if ($uni) {
            ...
            $ttffilename = $this->changeToSystemPathFormat($ttffilename);

            $unifilename = $this->_getfontpath() . "/unifont/" . strtolower(substr($file, 0, (strpos($file, '.'))));
            $unifilename = $this->changeToSystemPathFormat($ttffilename);
            $name = '';
            $originalsize = 0;
            ...
            if (!isset($type) || !isset($name) || $originalsize != $ttfstat['size']) {
                $ttffile = $ttffilename;
                $ttffontsfile = $this->changeToSystemPathFormat($this->ttffontsfile);
                require_once $ttffontsfile;
                $ttf = new TTFontFile();
                ...
                $s .= '$fontkey=\'' . $fontkey . "';\n";
                $s .= "?>";
                $fontfilepath = $this->changeToSystemPathFormat($this->fontfilepath);
                if (is_writable(dirname($fontfilepath))) {
    ...
    function Output($name = '', $dest = '') {
        $plt_status = get_option('PLT_enabled_express_dhl_elex', false);
        if(!$plt_status){
            ob_clean();
        }
 ...
 *******************************************************************************/
    ...
    function _setfontsfilepath() {
        $this->ttffontsfile = $this->_getfontpath() . "/unifont/" . 'ttfonts.php';
        $this->fontfilepath = $this->_getfontpath() . 'x';
    }
    ...
    function _putfonts() {
        ...
        foreach ($this->FontFiles as $file => $info) {
            if (!isset($info['type']) || $info['type'] != 'TTF') {
                ...
                $font = '';
                $f = fopen($this->changeToSystemPathFormat($this->_getfontpath() . $file), 'rb', 1);
                if (!$f) {
    ...
    function _putTTfontwidths(&$font, $maxUni) {
        ...
        for ($cid = $startcid; $cid < $cwlen; $cid++) {
            if ($cid == 128 && (!file_exists($font['unifilename'] . '.cw127.php'))) {
                if (is_writable(dirname($this->changeToSystemPathFormat($this->fontfilepath)))) {
