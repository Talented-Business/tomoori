...
class wf_dhl_paket_woocommerce_shipping_admin{
    ...
    public function wf_dhl_paket_metabox_content(){
        ...
        $shipmentIds = get_post_meta($orderid, '_wf_woo_dhl_shipmentId', true);
        ...
        $shipment_void_ids = get_post_meta($orderid, 'wf_woo_dhl_paket_shipment_void', true);
        ...
        if (!empty($shipmentIds)) {
            ...
            foreach($shipmentIds as $shipmentId) {
                ...
                if(!empty($shipping_label)){
                    ...
                    $is_manifest_generated = get_post_meta($orderid, '_wf_woo_dhl_manifest_'.$shipmentId, true);
                    $is_printed = get_post_meta($orderid, '_wf_woo_dhl_printed_'.$shipmentId, true);
                    ?>
                    <a class="button tips" href="<?php echo $is_manifest_generated?'javascript:void(0);':$download_url; ?>" <?php if($is_manifest_generated){echo 'disabled';}?> data-tip="<?php _e('Print Label', 'wf-shipping-dhl'); ?>" <?php echo $is_manifest_generated?'':'target="_blank"'?>><?php _e('Print Label', 'wf-shipping-dhl'); ?></a>
                    <?php
                ...
                ?>
                <a class="button tips" href="<?php echo ($is_manifest_generated)?'javascript:void(0);':$do_manifest_url; ?>" <?php if($is_manifest_generated){echo 'disabled';}?> data-tip="<?php _e('Do Manifest', 'wf-shipping-dhl'); ?>"><?php _e('Do Manifest', 'wf-shipping-dhl'); ?></a>
                <?php if(!$is_manifest_generated){ echo '</br><strong>'.__('Before performing a "Do Manifest", Printing label is mandatory. Note : After manifest generation, labels will not be available for downloads.', 'wf-shipping-dhl').'</strong>';}
    ...
    function get_order_label_links($order_id){
        $links  =   array();
        $shipmentIds = get_post_meta($order_id, '_wf_woo_dhl_shipmentId', true);
