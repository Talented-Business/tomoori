...
class wf_dhl_paket_woocommerce_shipping_admin_helper  {
    ...
    private function wf_get_receiver_details($package){
        ...
        $receiver['name1'] = elex_dhl_get_order_shipping_first_name($this->order).' '.elex_dhl_get_order_shipping_last_name($this->order);
        ...
        if($packstation_data){
            ...
            $address_line1 = explode(',', $address_line1);
            $address_line1_part1 = $address_line1_part2 = '';

            if(!empty($address_line1[0]) && is_numeric($address_line1[0])){
                $receiver['Address']['name3'] = $address_line1[0];
                $receiver['Address']['streetName'] = $address_line1[1];
                $address_line2 = '';
            }else if(!empty($address_line1[1]) && is_numeric($address_line1[1])){
                $receiver['Address']['name3']= $address_line1[1];
                $receiver['Address']['streetName'] = $address_line1[0];
                $address_line2 = '';
            }else{
                $streetNumber = (int) filter_var($address_line1[0], FILTER_SANITIZE_NUMBER_INT);
                if(!empty($streetNumber)){
                    $address_line2 = $streetNumber;
                    $address_line1[0] = preg_replace('/[0-9]/','',$address_line1[0]);
                }
                $receiver['Address']['streetName'] = $address_line1[0];
            }

            $receiver['Address']['streetNumber'] = !empty($address_line2)? mb_substr( $address_line2, 0, 5 ): ' ';// Used invisible character Alt + 255 to provide space, if address line 2 is empty because, normal whitespace is accepted by DHL Paket API
        ...
        if($this->settings['dhl_email_service'] == 'yes'){
            $receiver['Communication']['email'] = elex_dhl_get_order_billing_email($this->order);
        }

        $receiver['Communication']['contactPerson'] = elex_dhl_get_order_shipping_first_name($this->order).' '.elex_dhl_get_order_shipping_last_name($this->order);
        $receiver['Communication']['phone']         = elex_dhl_get_order_billing_phone($this->order);
    ...
    private function wf_get_export_doc($order_id, $dhl_packages){
        ...
        */      
        ...
        foreach($packed_products as $packed_product){
            ...
            $country_of_origin     = get_post_meta( $post_id, '_wf_manufacture_country', 1);
    ...
    private function wf_get_shipment_details($dhl_packages,$params  = array()){
        ...
        if($this->origin_country == 'DE' && $this->destination_country !='DE')
        {
            ...
            if($this->is_european_country($this->destination_country)){
                if($this->europaket_enabled){ // Europaket
                    $this->product_code = 'V54EPAK';
                }else if($this->dhl_connect_enabled ){//Paket Connect
    ...
    public function print_label_processor( $package ) {     
        ...
        if($this->shipmentErrorMessage){
            wf_admin_notice::add_notice(sprintf(__('Order #%d: %s','wf-shipping-dhl'), elex_dhl_get_order_id($this->order), $this->shipmentErrorMessage));
            return false;
        }else{
            return true;
        }
    ...
    private function process_result( $result = '' , $request, $dhl_packages) {
        if (!empty($result['ShipmentID']) && !empty($result['LabelImage'])){            
                    ...
                    foreach($shipment_services as $service_key=>$service_name){
                        if(is_array($this->custom_services)){
                            if(array_key_exists($service_key,$this->custom_services)){
                                update_post_meta($this->order_id,'_wf_woo_dhl_service_code',$service_key, false);
                                if($service_key == 'DeliveryOnTime'){
                                    update_post_meta($this->order_id,'_wf_woo_dhl_service_time',$shipment_services[$service_key]['time'], false);
                                }
                            }
                        }
            ...
            $stored_shipments_ids = get_post_meta($this->order_id, '_wf_woo_dhl_shipmentId', true);
            ...
            update_post_meta($this->order_id, '_wf_woo_dhl_shipmentId', $stored_shipments_ids, false);
            update_post_meta($this->order_id, '_wf_woo_dhl_shippingLabel_'.$shipmentId, $shippingLabel, true);
            update_post_meta($this->order_id,'_wf_woo_dhl_piece_information',$pieceInformation, false);

            if( !empty($result['ExportDoc']) ){
                update_post_meta($this->order_id, '_wf_woo_dhl_export_doc_'.$shipmentId, $result['ExportDoc'],true);
            }
    ...
    public function delete_shipment($order_id){
        ...
        $shipment_ids = get_post_meta($order_id,'_wf_woo_dhl_shipmentId', true);
        $client = $this->loginSoapClient();     
        $request = array();
    ...
    public function get_visual_check_of_age($item){
        return get_post_meta( elex_dhl_get_product_id($item), '_wf_dhlp_age_check', true);
    }
