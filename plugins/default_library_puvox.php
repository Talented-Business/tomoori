  ...
  trait default_methods__Puvox_Software
  {
		...
		*/
		...
		public function get_prefix_CHOSEN(){
			return ($this->newtork_managed ? $GLOBALS['wpdb']->base_prefix : $GLOBALS['wpdb']->prefix);
		}
		...
		*/
		...
		public function opts_page_output_parent($args=false)
		{
			...
			{ 
				...
				.networked_switcher_itsf { box-shadow: 0px 0px 15px #329c1e; z-index: 3; min-width:210px; background:#ffa637; border-radius: 190px 0 0  10px ;  border: 2px solid #80808094; padding:2px 10px 4px 30px; position: fixed; bottom: 0px; right: 0px;  line-height:1.2em; text-align: center; z-index: 77; }
				...
				<div class="networked_switcher_itsf_parent">
					<div class="networked_switcher_itsf">
						...
						<div class="modeChooser1">
							...
							<label for="networkwide" class="networkwide button<?php echo $this->newtork_managed ? "-primary":"";?> "><?php _e("Network dashboard (one settings page is for all subsites)");?></label>
							<input id="networkwide" onchange="managed_from_onchanger(this)" type="radio" name="<?php echo $this->slug;?>[managed_from_site]" value="network" <?php checked($this->newtork_managed);?> />
							<label for="subsite" class="subsite button<?php echo !$this->newtork_managed ? "-primary":"";?>"><?php _e("Subsite dashboards (each sub-site has it's own settings page)");?></label>
							<input id="subsite" onchange="managed_from_onchanger(this)"  type="radio" name="<?php echo $this->slug;?>[managed_from_site]" value="singlesite" <?php checked(!$this->newtork_managed);?>  />
							<!--					
							<input type="submit" name="<?php echo $this->slug;?>[managed_from_site]" value="<?php _e("Network-Wide");?>" class="button<?php echo $this->newtork_managed ? "-primary":"";?>" />
							<input type="submit" name="<?php echo $this->slug;?>[managed_from_site]" value="<?php _e("Per sub-site, individually");?>" class="button<?php echo !$this->newtork_managed? "-primary":"";?>" />
							-->
			...
			if ( (is_network_admin() && $this->newtork_managed) || (!is_network_admin() && !$this->newtork_managed) ){
				...
				echo '<div style="display: flex; background: white; flex-direction: column; max-width: 600px; margin: 100px auto; border-radius: 10px; padding: 30px;"><h1>'.__('Plugin is set to be managed per: <span class="perChosen">'. ($this->newtork_managed ? "Network": "Sub-sites") ).'</span></h1></div>';
			}
		}
