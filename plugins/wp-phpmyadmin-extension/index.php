...
namespace WpPhpMyAdminExtension
{
  ...
  class PluginClass
  {
	...
	public function setup_files()
	{
		...
		$this->pma_delete_dirs		= array('/vendor/tecnickcom/tcpdf', '/locale', '/theme/original', '/doc', '/setup', '/examples', '/install', '/js/vendor/openlayers' );	  //vendor\phpmyadmin\sql-parser\locale
	...
	private function download_install_pma()
	{
		if( is_dir($this->pma_abspath_real) && !is_dir($this->pma_abspath) )
		{
			...
			$this->locker	= fopen( $this->lockFile=WP_CONTENT_DIR."/lock.txt","w+"); 
			...
			{ 
				if( is_dir($this->pma_abspath_real) && !is_dir($this->pma_abspath) ) //check again after LOCK 
				{
					...
					if(is_admin())
					{
						...
						if(!file_exists($this->path_to_pma_config) || $force)
						{
							...
							//$content = str_replace( '___DBARRAY___',			'array($_COOKIE["pma_DB_NAME"])',	$content);  
							$content = str_replace( '___DBARRAY___',			'[file_get_contents(__DIR__."/_session_temp_db_name_".$_SERVER["HTTP_HOST"].".php")]',	$content);  
					...
					$common_inc_content_new  = $this->str_replace_first( '<?php', '<?php '.file_get_contents($this->path_to_def_common) , file_get_contents($this->path_to_pma_common) );
				...
				flock($this->locker,LOCK_UN); if (file_exists($this->lockFile)) unlink($this->lockFile);
			} //end locker
		}
	}
